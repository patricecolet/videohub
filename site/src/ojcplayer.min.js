var player = (function(){var play = $("#play"), fullscreen = $("#expand"), playlist = $("#playlist"), audioVolume = $("#audioVolume"), progress = $('#progress'), start = $('#start'), finish = $('#finish'), preload = $('#preload'); var mediaPlayer, currentTrack = "0", videos, videoNodes; play.on("click", (e) => {if (mediaPlayer.paused || mediaPlayer.ended){e.target.setAttribute("class", "fas fa-pause"); mediaPlayer.play(); } else{mediaPlayer.pause(); e.target.setAttribute("class", "fas fa-play"); }}); audioVolume.on("click", (e) => {if (mediaPlayer.muted){e.target.setAttribute('class', 'fas fa-volume-up')
        mediaPlayer.muted = !mediaPlayer.muted; } else{e.target.setAttribute('class', 'fas fa-volume-off')
        mediaPlayer.muted = !mediaPlayer.muted; }}); fullscreen.on("click", () => {if (mediaPlayer.requestFullscreen)mediaPlayer.requestFullscreen();  else if (mediaPlayer.webkitRequestFullscreen)mediaPlayer.webkitRequestFullscreen();  else if (mediaPlayer.mozRequestFullScreen)mediaPlayer.mozRequestFullScreen();  else if (mediaPlayer.msRequestFullscreen)mediaPlayer.msRequestFullscreen(); }); playlist.on("click", function(e){var target = e.target; if (target.id != "playlist"){var id = target.closest(".video").id, el = target.closest(".video"); preload.classList.add('lds-ring'); higlight(id, currentTrack.toString()); set(videos[id].source)
        mediaPlayer.on('canplay', videoIsReady)}}); function set(video){mediaPlayer.src = video; }
...
function loadList(playlist){var domPlayList = $('#playlist'), fragmentList = document.createDocumentFragment(); for (let video of playlist)
...
var getVideos = function(){getJSON('../source/videos.json', function(response){videos = response; loadList(videos)
...
return{init:function(){mediaPlayer = $("#media-video"); mediaPlayer.controls = false; mediaPlayer.muted = true; getVideos(); mediaPlayer.on('canplay', videoIsReady)
        mediaPlayer.on('timeupdate', handleProgress)
        mediaPlayer.on('click', togglePlay)}}})();